...
from src.features.user_management.shared.token_generator import TokenGenerator, TokenRequest
...
class LoginHandler:
    ...
    async def handle(self, request: LoginRequest) -> LoginResponse:
        ...
        if not user:
            return LoginResponse(is_successful=False, token="", expiration_time=0)
        
        if not self.password_hasher.verify_password(request.password, user.password_hashed):
            return LoginResponse(is_successful=False, token="", expiration_time=0)
        
        token_response =  self.token_generator.generate_token(TokenRequest(
