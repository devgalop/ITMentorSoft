...
async def test_when_token_is_valid_and_user_exists_then_password_is_changed():
    ...
    user_recovery_token_repository.get_user_id_by_transaction_id = AsyncMock(return_value=user_recover_info)
    ...
    handler = ChangePasswordHandler(user_repository, user_recovery_token_repository, password_hasher)
    response = await handler.handle(ChangePasswordRequestWithToken(
        token="valid_token",
        id_trx="valid_id_trx",
    ...
    user_recovery_token_repository.get_user_id_by_transaction_id.assert_called_once_with("valid_id_trx")
    password_hasher.verify_password.assert_called_once_with("valid_token", "hashed_token")
    user_repository.get_user_by_id.assert_called_once_with("user123")
    user_recovery_token_repository.revoke_tokens_by_user_id.assert_called_once_with("user123")
    password_hasher.hash_password.assert_called_once_with(new_pass)
    user_repository.change_password.assert_called_once_with("user123", "hashed_new_password")
...
async def test_when_token_is_invalid_then_should_respond_with_error():
    ...
    user_recovery_token_repository.get_user_id_by_transaction_id.assert_called_once_with("valid_id_trx")
    password_hasher.verify_password.assert_called_once_with("invalid_token", "hashed_token")
...
async def test_when_token_is_different_then_should_respond_with_error():
    ...
    assert response.message == "Invalid or expired token"
    user_recovery_token_repository.get_user_id_by_transaction_id.assert_called_once_with("valid_id_trx")
    password_hasher.verify_password.assert_called_once_with("different_token", "hashed_token")
    user_repository.get_user_by_id.assert_not_called()
    user_recovery_token_repository.revoke_tokens_by_user_id.assert_not_called()
    password_hasher.hash_password.assert_not_called()
    user_repository.change_password.assert_not_called()
