...
async def test_when_email_does_not_exist_then_response_message_is_returned():
    ...
    assert response.message == "If the email exists in our system, you will receive a password recovery email shortly."
    user_repository.get_user_response_by_email.assert_called_once_with("nonexistent@example.com")
...
async def test_when_email_exists_then_recovery_process_is_initiated():
    user_repository = AsyncMock(spec=UserRepository)
    user_repository.get_user_response_by_email.return_value = AsyncMock(id=1, username="testuser")
    token_generator = AsyncMock(spec=TokenGenerator)
    token_generator.generate_random_token.return_value = AsyncMock(token="random-token", expiration_time=1234567890)
    user_recovery_repository = AsyncMock(spec=UserRecoveryTokenRepository)
    notification_service = AsyncMock(spec=NotificationService)
    password_hasher = AsyncMock(spec=PasswordHasher)
    ...
    assert isinstance(response, RecoveryPasswordResponse)
    assert response.message == "If the email exists in our system, you will receive a password recovery email shortly."
    user_repository.get_user_response_by_email.assert_called_once_with("testuser@example.com")
